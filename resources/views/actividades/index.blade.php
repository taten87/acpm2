<x-app-layout>
    ...
    <div class="py-12 min-h-screen bg-slate-950 text-slate-100" x-data="{
                ...
                <form method="POST" action="{{ route('actividades.store') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="descripcion"
                            class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Descripción
                            de la Actividad</label>
                        <textarea id="descripcion" name="descripcion" rows="3" required
                            placeholder="Escribe la descripción de la actividad..."
                            class="w-full bg-slate-950/60 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 rounded-xl p-3 text-sm text-slate-100 placeholder-slate-500 transition-all resize-y">{{ old('descripcion') }}</textarea>
                    </div>
                        ...
                        <tbody class="divide-y divide-slate-800/60 bg-slate-900/30">
                            @forelse ($actividades as $actividad)
                                <tr class="hover:bg-slate-800/40 transition-colors">
                                    <td
                                        class="px-6 py-4 font-normal text-slate-100 whitespace-pre-line leading-relaxed">
        ...
        <div x-show="openEditModal"
            ...
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-2xl max-w-lg w-full relative">
                <h3 class="text-lg font-bold text-slate-100 mb-4">Editar Actividad de Proyecto</h3>
                ...
                <form :action="editUrl" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Descripción</label>
                        <textarea name="descripcion" x-model="editDescripcion" rows="4" required
                            class="w-full bg-slate-950/60 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 rounded-xl p-3 text-sm text-slate-100 transition-all resize-y"></textarea>
                    </div>
        ...
        <div x-show="openDeleteModal"
            ...
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-2xl max-w-md w-full relative">
                <h3 class="text-lg font-bold text-rose-400 mb-2">¿Eliminar actividad de proyecto?</h3>

                <p class="text-xs font-medium text-slate-300 mb-5 bg-slate-950/80 border border-slate-800/80 p-3 rounded-xl italic break-words max-h-32 overflow-y-auto leading-relaxed"
